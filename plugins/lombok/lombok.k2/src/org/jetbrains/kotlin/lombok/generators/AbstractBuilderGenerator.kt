...
abstract class AbstractBuilderGenerator<T : AbstractBuilder>(session: FirSession) : FirDeclarationGenerationExtension(session) {
    ...
    private fun addBuilderCallables(
        builderSymbol: FirClassSymbol<*>,
        existingFunctionNames: Set<Name>,
        existingVariableNames: MutableSet<Name>,
    ) {
    ...
    private fun MutableMap<Name, FirNamedFunctionSymbol>.addEntityMethods(
        ...
        for ((val builder, val builderDeclaration = declaration) in builderWithDeclarations) {
            ...
            // Using of `scopeProvider` isn't safe for Kotlin classes (it causes infinite recursion), but it's safe for Java classes.
            // On the other side, FIR declarations don't contain nested classes for Java classes, but they do for Kotlin classes.
            // That'd why use the different logic for getting an existing builder if it exists.
    ...
    protected fun getBuilderNames(classSymbol: FirClassSymbol<*>): Set<Name> = buildSet {
        ...
        // We can't rely on scope providers during nested classifier names extraction because it causes infinite recursion.
        // The only way is to get them via direct access.
        // However, use different logic for Java and Kotlin because:
        //   * In Java names aren't available via FIR declarations, but they can be obtained from `existingNestedClassifierNames`
        //   * In Kotlin they are available via FIR declarations, but regular classes don't have `existingNestedClassifierNames`
        val classFir = classSymbol.fir
        val existingClassifierNames = if (classFir is FirJavaClass) {
            classFir.existingNestedClassifierNames.toSet()
        } else {
            buildSet {
