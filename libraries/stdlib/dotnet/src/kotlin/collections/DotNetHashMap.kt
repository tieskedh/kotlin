...
public actual class HashMap<K, V> private constructor(
    ...
    public actual constructor(initialCapacity: Int) : this(
        dotNetHashObjectArray(initialCapacity),
        null,
    ...
    override actual fun put(key: K, value: V): V? {
        ...
        if (index < 0) {
            val oldValue = valueAt(valuesArray, -index - 1)
    ...
    override actual fun clear() {
        ...
        clearDotNetHashObjectRange(keysArray, 0, length)
        valuesArray?.let { clearDotNetHashObjectRange(it, 0, length) }
        _size = 0
        length = 0
    ...
    private fun keyAt(index: Int): K = keysArray[index] as K

    @Suppress("UNCHECKED_CAST")
    private fun valueAt(index: Int): V = valuesArray!![index] as V

    @Suppress("UNCHECKED_CAST")
    private fun valueAt(values: Array<Any?>, index: Int): V = values[index] as V
    ...
    private fun ensureCapacity(minCapacity: Int) {
        ...
        if (minCapacity > this.capacity) {
            ...
            keysArray = keysArray.copyOf(newSize)
            valuesArray = valuesArray?.copyOf(newSize)
    ...
    private fun allocateValuesArray(): Array<Any?> {
        ...
        val newValuesArray = dotNetHashObjectArray(capacity)
        valuesArray = newValuesArray
        return newValuesArray
    }

    // The erased owner stores every key in object[]. Null still hashes to zero, exactly as in Common.
    private fun hash(key: Any?) = if (key == null) 0 else (key.hashCode() * MAGIC) ushr hashShift
    ...
    private fun compact(updateHashArray: Boolean) {
        ...
        clearDotNetHashObjectRange(keysArray, j, length)
        valuesArray?.let { clearDotNetHashObjectRange(it, j, length) }
        length = j
...
//        require(newHashSize > hashSize) { "Rehash can only be executed with a grown hash array" }
    ...
    internal fun addKey(key: K): Int {
        ...
        retry@ while (true) {
            ...
            val doubledProbeDistance = maxProbeDistance * 2
            val tentativeMaxProbeDistance =
                if (doubledProbeDistance < hashSize / 2) doubledProbeDistance else hashSize / 2
            var probeDistance = 0
            while (true) {
                val index = hashArray[hash]
    ...
    private companion object {
        ...
        private fun computeHashSize(capacity: Int): Int = dotNetHighestOneBit(capacity.coerceAtLeast(1) * 3)

        private fun computeShift(hashSize: Int): Int = dotNetCountLeadingZeroBits(hashSize) + 1

        private fun dotNetHashObjectArray(size: Int): Array<Any?> {
            if (size < 0) throw IllegalArgumentException("Negative initial capacity: $size")
            return arrayOfNulls<Any?>(size)
        }

        private fun clearDotNetHashObjectRange(array: Array<Any?>, fromIndex: Int, toIndex: Int) {
            var index = fromIndex
            while (index < toIndex) array[index++] = null
        }

        private fun dotNetHighestOneBit(value: Int): Int {
            var result = value
            ...
            return result - (result ushr 1)
        }

        private fun dotNetCountLeadingZeroBits(value: Int): Int {
            if (value == 0) return Int.SIZE_BITS
            var count = 0
            var bit = Int.MIN_VALUE
    ...
    internal class KeysItr<K, V>(map: HashMap<K, V>) : Itr<K, V>(map), MutableIterator<K> {
        override fun next(): K {
            ...
            lastIndex = index++
            val result = map.keyAt(lastIndex)
            initNext()
            return result
        }
    ...
    internal class ValuesItr<K, V>(map: HashMap<K, V>) : Itr<K, V>(map), MutableIterator<V> {
        override fun next(): V {
            ...
            if (index >= map.length) throw NoSuchElementException()
            lastIndex = index++
            val result = map.valueAt(lastIndex)
            initNext()
            return result
        }
    }
    ...
    internal class EntriesItr<K, V>(map: HashMap<K, V>) : Itr<K, V>(map),
        ...
        internal fun nextHashCode(): Int {
            if (index >= map.length) throw NoSuchElementException()
            lastIndex = index++
            val result = (map.keysArray[lastIndex]?.hashCode() ?: 0) xor
                    (map.valuesArray!![lastIndex]?.hashCode() ?: 0)
            initNext()
            return result
        }
    ...
    internal class EntryRef<K, V>(
        ...
        override fun setValue(newValue: V): V {
            ...
            val oldValue = map.valueAt(valuesArray, index)
