...
// user-reachable shape of the widening arm. Identity between two UNRELATED interface types now
// widens both references to System.Object, the physical Kotlin Any root, and remains a plain
// `ceq`; `unrelated` pins that fallback. `Marker` is a SEALED interface: sealedness is pure
// frontend-enforced metadata (JVM precedent — the JVM backend emits an ordinary interface too),
// so it is deliberately accepted and emitted as a plain `.class interface`, pinned here including
// dispatch through the sealed-interface-typed local.
interface Root
...
fun main() {
    ...
    println(unrelated(LeafA(), OtherLeaf()))
    val m: Marker = Marked()
    println(m.tag())
}
