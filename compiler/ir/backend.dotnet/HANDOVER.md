...
- The generated Common collection terminal-operation slice is committed and pushed as
  `e0b835402`. The bounded architecture correction is committed and pushed as `70c579496`; it
...
- The accepted C# source-authoring direction now has its first DLL-owned manifest slice. Every
  ...
  payload into a true managed resource before schema/package freeze. Multiple-parent,
  cross-profile helper-to-DIM, and non-generic inheritance remain unsupported; intersections,
  constraints, inherited mutable-property obligations, special barriers, friend-accessible
  internal interfaces, and runtime-bootstrap interfaces are the next schema coverage, followed by
  the actual Roslyn generator/analyzer. The general portable CLR-surface comparer excludes only
  this profile-specific carrier; the manifest test decodes and compares its logical/helper
  identities separately. The strict baseline becomes 847/0/0/0 across 16 XML suites.
...
- The same fixture now splits those obligations across `ShapeParent<out T>` and
  `Shape<out T> : ShapeParent<T>`. Both same-DLL declarations own separate manifest contracts;
  ordinary CLR interface metadata remains authoritative for the edge and substitution. A C#
  partial type implements the child exact view, supplies parent and child canonical adapters,
  forwards the parent helper on portable profiles, and inherits the parent DIM on `net10.0`.
  Kotlin executes parent properties/defaults and child methods through the child view on all three
  profiles. This first slice admitted one same-DLL generic parent; the later entries below extend
  it to cross-assembly and multi-parent generic graphs. Non-generic parents remain unsupported.
- Cross-assembly single-parent composition now uses the same schema. The `netstandard2.0` and
  `net10.0` lanes compile `ShapeParent` and `Shape` into separate Kotlin libraries, bind the child
  through the existing physical KLIB index, then delete both KLIBs. The generated C# compilation
...
- Cross-profile composition now compiles the parent graph for `netstandard2.0` and
...
- Multi-parent generic composition now uses the same schema/CLR split. The manifest fixture forms
  `Shape` from two generic branches sharing `ShapeRoot`; logical root keys deduplicate its
...
- C# implementation manifest schema 2 now represents that derived intersection. It records the
  ...
  schema 1.
- Schema 2 now also covers the split mutable-property intersection. Its declared view records only
  the variance-safe getter; its exact view records the getter and setter against one
...
- C# implementation manifest schema 7 records the existing
  ...
  share one Kotlin declaration-identity table. Records carry the number of leading
  ...
  no policy and retains cast failure. `Kotlin.Runtime.dll` now publishes complete built-in-derived
...
- The production C# authoring component now begins under `csharp-authoring` as a locked
  ...
  adapters. A non-partial container is `KDNCS011`; file-local types fail explicitly rather than
  receiving an invalid generated part. Value-type implementors are an accepted deferral and
  remain `KDNCS010`: no implicit wrapper is permitted while portable helper boxing and modern
  DIM/constrained dispatch lack a profile-uniform Kotlin identity, copy, mutation, and default
  dispatch contract. Reopening that boundary requires the cross-profile matrix in the
  source-authoring ADR. Roslyn source hints now hash the fully qualified C# type, preventing
  punctuation/nesting filename collisions without creating a persisted declaration identity. The
...
- The CLR annotation-interoperability audit and first foreign FIR slice are implemented. The
  ...
  integration tests).
- The first exact Kotlin-contract/CodeAnalysis overlap is implemented.
  `DotNetClrNotNullWhenMetadataDecoder` recognizes an exact selected-graph top-level
  `System.Diagnostics.CodeAnalysis.NotNullWhenAttribute` with one Boolean constructor, preserves
  absent/decoded/structured-invalid results, and rejects duplicate, malformed, or named payloads.
  The foreign provider attaches only common FIR
  `returns(true|false) implies (parameter != null)` effects to Boolean-returning methods and
  string/object parameters. It does not change declaration nullability or infer the inverse.
  Roslyn and hostile polyfill fixtures prove both valid branches, both invalid inverses, absence,
  duplicates, a wrong constructor, a named payload, a corrupted prolog, a non-reference
...
- The exact unconditional non-return view is implemented.
  `DotNetClrDoesNotReturnMetadataDecoder` recognizes only exact selected-graph
  `System.Diagnostics.CodeAnalysis.DoesNotReturnAttribute()` evidence on a MethodDef. The
  provider exposes logical Kotlin `Nothing` for every physical return shape in the closed grammar
  while retaining the original MethodDef signature. This follows Kotlin Common reachability:
  common FIR does not currently make `returns() implies false` continuation unreachable, whereas
  `Nothing` does and already receives the mature common `KotlinNothingValueException` guard.
  Real Roslyn `void` and `int` methods prove null-branch and Elvis reachability,
  `() -> Nothing` callable references, and bottom-type subtyping. An unannotated control,
  duplicates, wrong constructors, named payloads, a return-target look-alike, and a corrupted
...
- Exact method-level, imported-interface TypeDef, Property, and accessor CLR deprecation are
  ...
  omitted/null messages use `Deprecated in .NET`, false/default maps to `WARNING`, and true maps
  to `ERROR`, never `HIDDEN`. Warning declarations remain resolvable; method calls still codegen
...
- The first Common/actual stdlib source-product slice removes the target
  `UsedFromCompilerGeneratedCode` mirror and compiles the exact Common `Annotations.kt` plus
  exhaustive-when expect source beside the .NET actual helper. The CLI and test harness now create
  distinct Common/platform FIR sessions only for classified Common inputs, and library metadata
  serialization moved after FIR2IR actualisation through `Fir2KlibMetadataSerializer`. Explicit
  stdlib products reject the complete file set when the Common files are misclassified as
  platform sources. FIR2IR's exhaustive-when lookup had a latent expect+actual bug:
  `singleOrNull()` saw both and silently used the old parameterless builtin. It now follows the
  existing built-in-symbol rule and prefers the non-expect declaration. Do not accept a golden
  that drops the `Kotlin.Stdlib` helper call; that is a semantic regression. Net10 and net48
  direct/fallback product equality, installed consumption, both PSI/LightTree IL pins, and the
  full net48 assembler/runtime matrix pass in focused runs. The full Common `ExceptionsH.kt`
...
- The first Common collections programme slice replaces the handwritten terminal-operation
  ...
  integration tests).
- The second Common collections slice extracts the complete Common
  `Array<out T>.asList(): List<T>` declaration and emits its actual once on
  `Kotlin.Collections.CollectionsKt`. A private ordinary Kotlin `List<T>, RandomAccess` view keeps
  the exact original array and backed sub-views; it is neither copied into a BCL collection nor
  exposed as one. The embedded KLIB retains the output projection, the physical index exposes only
  the public facade method, and reference-array covariance is admitted without pretending that
  `int32[]` can widen to `object[]`. Direct/fallback identity, separate and installed binding,
  portable Framework CLR 4/CoreCLR 10 execution, hostile widened arguments, structural behavior,
