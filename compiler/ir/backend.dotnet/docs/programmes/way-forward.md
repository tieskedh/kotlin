...
source-built Stdlib census. Six bounded local-placement forms are executable
...
may have one identity-preserving `STRICT_OWNER_INPUT(!m)` and no MethodSpec. Local
...
multiple ordinary arguments, MethodSpec calls, multiple consumers, control-flow
...
The next sequence is: extend split-pair retention to justified MethodSpec calls,
multiple consumers, and control-flow joins; close multiple ordinary inputs and
remaining parameter-entry forms; then prove null/bottom/unknown joins and
