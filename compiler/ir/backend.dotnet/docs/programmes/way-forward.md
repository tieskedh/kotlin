...
erased `Iterator` calls now need no call-site rerouting. It does not close
body-generated operations whose exact/capability choice depends on their
receiver or value provenance. Those still require the final idempotent router.
This is a completeness gate for that router, not permission for the emitter to
infer representation ad hoc.
