...
generated by `ForLoopsLowering` must be classified from the same published
contract as a source call rather than escaping an earlier `IrCall`-identity
table. This is a completeness gate for the late router, not permission for the
emitter to infer representation ad hoc.
