...
producer, the exact one-producer/one-consumer invariant-cell method bundle, and
its exact mutable-property equivalent are capability-optional exceptions. A C#
type implementing only such admitted invariant contracts is already complete
with the natural `I<T>` and need not be partial; exact calls stay natural and
Kotlin projected operations have a cached unique-construction fallback. For
the property family, both natural slots must share one non-null Property name,
while both semantic slots must remain ordinary compiler MethodDefs without a
Property row. The analyzer/generator ignores that type only after the manifest
...
portable static default helper prepends the interface owner parameters before
the method parameters. For an abstract root, C# authors implement only the
ordinary natural generic method; generated source connects the semantic slot
to that same method. For a default, generated C# forwards its own `R`
...
member consumer root. Both natural and semantic MethodDefs, every helper, and
each generated slot or bridge retain that constructed GenericParamConstraint
...
MethodImpl on Framework. Roslyn source and metadata parameters are alpha-
equivalent by method-parameter kind and ordinal inside the otherwise exact
recursive constraint type; source-symbol identity is not part of the CLR
contract. Owner-relative, other nominal or constructed, special, and multiple
forms remain outside this rehearsal gate.
