...
internal class DotNetIlEmitter(
    ...
    fun emit(moduleFragment: IrModuleFragment): DotNetIlEmissionResult? {
        ...
        val genericOwnerObjectStateFields = if (genericOwnerRehearsal) {
            genericOwnerArchitecturePlans.values
                ...
                .flatMapTo(linkedSetOf()) { plan ->
                plan.stateCarriers.values.mapNotNull { state ->
                    state.field.takeIf {
        ...
        val boundExternalGenericOwnerPhysicalSlots = buildMap {
            putAll(externalGenericOwnerPhysicalSlots)
            genericOwnerCapabilitySlots.forEach { entry ->
                val source = entry.key
                val slot = entry.value
                externalDeclarations.genericOwnerMemberFamilyOrNull(source)?.let { binding ->
                    put(slot, DotNetBoundGenericOwnerPhysicalSlot(
                        binding.library,
                        binding.family,
            ...
            genericOwnerDefaultCapabilitySlots.forEach { entry ->
                val source = entry.key
                val slot = entry.value
                externalDeclarations.genericOwnerMemberFamilyOrNull(source)?.let { binding ->
                    binding.family.defaultCapabilityMethodName?.let { methodName ->
                        put(slot, DotNetBoundGenericOwnerPhysicalSlot(
                            binding.library,
                            binding.family,
                            binding.family.ownerPath,
        ...
        // A bootstrap stdlib is emitted before this user assembly and is included in
        // externalDeclarations through its completed producer index. Its lowering entries are
        // still the same IR symbols, so bind those symbols to the producer-recorded physical
        // initialization owner just as a separately deserialized consumer would.
        for ([logicalOwner, lowered] in staticInitializations) {
            if (lowered.entry in availableFunctions) continue
            val binding = externalDeclarations.staticInitializationOrNull(logicalOwner) ?: continue
            availableFunctions[lowered.entry] = externalDeclarations.staticInitializationFunctionInfo(
                lowered.entry,
                binding,
                typeMapper,
            )
            ...
            for (function in availableFunctions.keys.toList()) {
                ...
                try {
                    ...
                    val functionInfo = DotNetIlFunctionInfo(
                        ...
                        availableFunctions.getValue(function).genericOwnerPhysicalMethodIdentity,
                    )
        ...
        val declarations = collectDotNetLibraryDeclarations(
            ...
            genericOwnerCapabilitySlots = if (genericOwnerRehearsal) genericOwnerCapabilitySlots else emptyMap(),
        ...
        if (!genericOwnerRehearsal) {
            ...
            check(renderedClasses.values.all { rendered ->
                    rendered.typeDefEmissionObservations.isEmpty() &&
                            rendered.methodImplObservations.isEmpty()
                } && renderedMethods.values.all { rendered -> rendered.methodImplObservations.isEmpty() } &&
                    renderedStaticInitializers.values.all { rendered ->
                        rendered.methodImplObservations.isEmpty()
                    }
            ) {
                "production IL emission must not retain raw generic-owner complete-liveness observations"
            }
        }
        ...
        val rawTypeDefEmissionObservations = if (genericOwnerRehearsal) {
            renderedClasses.values.flatMap { rendered -> rendered.typeDefEmissionObservations }
        } else {
            emptyList()
        }
        val rawMethodImplObservations = if (genericOwnerRehearsal) {
            buildList {
                renderedClasses.values.forEach { rendered -> addAll(rendered.methodImplObservations) }
                renderedMethods.values.forEach { rendered -> addAll(rendered.methodImplObservations) }
                renderedStaticInitializers.values.forEach { rendered ->
                    addAll(rendered.methodImplObservations)
                }
            }
        } else {
            emptyList()
        }
        val localTypeDefCandidatesByClassInfo = java.util.IdentityHashMap<
                DotNetIlClassInfo,
                MutableList<DotNetGenericOwnerObservedLocalTypeDefCandidate>,
                >()
        val localTypeDefPhysicalKeys = java.util.IdentityHashMap<
                DotNetIlClassInfo,
        ...
        fun recordLocalTypeDefIdentity(
            ...
            localTypeDefPhysicalKeys.getOrPut(classInfo) {
                DotNetGenericOwnerObservedPhysicalTypeDefKey(localTypeDefPhysicalKeys.size)
            }
            localTypeDefCandidatesByClassInfo
                .getOrPut(classInfo) { mutableListOf() }
                .add(DotNetGenericOwnerObservedLocalTypeDefCandidate(
                    identity = identity,
        ...
        if (genericOwnerRehearsal) {
            availableClasses.forEach { entry ->
                ...
                val category = if (owner.isInterface) {
                    DotNetGenericOwnerPhysicalNamedTypeCategory.INTERFACE
                } else {
                    DotNetGenericOwnerPhysicalNamedTypeCategory.CLASS
                }
                val aliasedViews = buildList {
                    if (interfaceInfo?.canonicalClassInfo === classInfo) {
                        add(DotNetGenericInterfaceView.CANONICAL)
                    }
                    if (interfaceInfo?.declaredClassInfo === classInfo) {
                        add(DotNetGenericInterfaceView.DECLARED)
                    }
                    if (isEmpty()) add(null)
                }
                aliasedViews.forEach { view ->
        ...
        val localTypeDefsByClassInfo = java.util.IdentityHashMap<
                ...
                >().apply {
            localTypeDefCandidatesByClassInfo.forEach { entry ->
                put(
                    entry.key,
                    resolveDotNetGenericOwnerObservedLocalTypeDef(
                        checkNotNull(localTypeDefPhysicalKeys[entry.key]),
                        entry.value,
                    ),
                )
            }
            val duplicateKeys = conflictingDotNetGenericOwnerObservedPhysicalTypeDefKeys(
                values.filterIsInstance<DotNetGenericOwnerObservedLocalTypeDefResolution.Known>()
                    .map { resolution -> resolution.typeDef },
            )
            if (duplicateKeys.isNotEmpty()) {
                entries.toList().forEach { entry ->
                    val known = entry.value as?
                            DotNetGenericOwnerObservedLocalTypeDefResolution.Known ?: return@forEach
                    if (known.typeDef.physicalKey in duplicateKeys) {
                        put(
                            entry.key,
                            DotNetGenericOwnerObservedLocalTypeDefResolution.Conflict(
                                "one logical TypeDef alias was attached to multiple emitted physical TypeDefs",
                            ),
                        )
                    }
                }
        ...
        val localTypeDefByClassInfo = java.util.IdentityHashMap<
            ...
            localTypeDefsByClassInfo.forEach { entry ->
                (entry.value as? DotNetGenericOwnerObservedLocalTypeDefResolution.Known)
                    ?.typeDef
                    ?.let { typeDef -> put(entry.key, typeDef.identity) }
            }
        }
        ...
        fun normalizeObservedMethodCarrier(
            ...
            is DotNetIlValueType.UserClass -> {
                when (val resolution = localTypeDefsByClassInfo[carrier.classInfo]) {
                    null -> DotNetGenericOwnerObservedMethodCarrier.Other(
                    ...
                    is DotNetGenericOwnerObservedLocalTypeDefResolution.Known -> when {
        ...
        val methodDefHeaderObservations = rawMethodDefHeaderObservations.map { raw ->
            ...
            val physicalOwner = (ownerResolution as?
                    DotNetGenericOwnerObservedLocalTypeDefResolution.Known)?.typeDef
            val observedOwner = when (ownerResolution) {
                ...
                is DotNetGenericOwnerObservedLocalTypeDefResolution.Known ->
                    DotNetGenericOwnerObservedMethodDefOwner.Local(ownerResolution.typeDef)
            }
        ...
        fun normalizeObservedTypeCarrier(
            carrier: DotNetIlValueType,
            physicalBinder: DotNetGenericOwnerObservedLocalTypeDef?,
        ): DotNetGenericOwnerObservedMethodCarrier = when (carrier) {
            DotNetIlValueType.Boolean -> DotNetGenericOwnerObservedMethodCarrier.Leaf(
                DotNetGenericOwnerPhysicalTypeKind.BOOLEAN,
            )
            ...
            is DotNetIlValueType.TypeParameter -> when {
                carrier.isMethodParameter -> DotNetGenericOwnerObservedMethodCarrier.Unbindable(
                    "a TypeDef or MethodImpl owner cannot contain a method parameter",
                    isConflict = true,
                )
                physicalBinder == null -> DotNetGenericOwnerObservedMethodCarrier.Unbindable(
                    "an emitted owner parameter has no local physical binder",
                    isConflict = false,
                )
                carrier.index !in 0 until physicalBinder.genericArity ->
                    DotNetGenericOwnerObservedMethodCarrier.Unbindable(
                        "an emitted owner-parameter index exceeds its physical TypeDef arity",
                        isConflict = true,
                    )
                else -> DotNetGenericOwnerObservedMethodCarrier.OwnerParameter(
                    physicalBinder,
            ...
            is DotNetIlValueType.UserClass -> when (val resolution =
                localTypeDefsByClassInfo[carrier.classInfo]
            ) {
                null -> DotNetGenericOwnerObservedMethodCarrier.Other(carrier.nameInSignature)
                is DotNetGenericOwnerObservedLocalTypeDefResolution.Conflict ->
                    DotNetGenericOwnerObservedMethodCarrier.Unbindable(
                        resolution.reason,
                        isConflict = true,
                    )
                is DotNetGenericOwnerObservedLocalTypeDefResolution.Known -> if (
                    carrier.classInfo.typeParameterCount == 0
                ) {
                    ...
                    DotNetGenericOwnerObservedMethodCarrier.Unbindable(
                        "an emitted open generic TypeDef was used without a construction",
                        isConflict = true,
                    )
                }
            }
            is DotNetIlValueType.GenericInstance -> when (val resolution =
                localTypeDefsByClassInfo[carrier.classInfo]
            ) {
                null -> DotNetGenericOwnerObservedMethodCarrier.Other(carrier.nameInSignature)
                is DotNetGenericOwnerObservedLocalTypeDefResolution.Conflict ->
                ...
                is DotNetGenericOwnerObservedLocalTypeDefResolution.Known -> if (
                    ...
                    DotNetGenericOwnerObservedMethodCarrier.LocalConstruction(
                        resolution.typeDef,
                        carrier.arguments.map { argument ->
                            normalizeObservedTypeCarrier(argument, physicalBinder)
                        },
                    )
                } else {
                    DotNetGenericOwnerObservedMethodCarrier.Unbindable(
                        "an emitted generic construction disagrees with its physical TypeDef arity",
                        isConflict = true,
                    )
                }
            }
            ...
            else -> DotNetGenericOwnerObservedMethodCarrier.Other(carrier.nameInSignature)
        }
        val typeDefEmissionObservations = rawTypeDefEmissionObservations.map { raw ->
            val owner = observedOwner(raw.physicalType)
            val physicalBinder = (owner as? DotNetGenericOwnerObservedMethodDefOwner.Local)?.typeDef
            val checkedOwner = if (physicalBinder != null &&
                (physicalBinder.category != raw.category ||
                        physicalBinder.genericArity != raw.genericParameters.size)
            ) {
                DotNetGenericOwnerObservedMethodDefOwner.Unbindable(
                    "the emitted TypeDef category or GenericParam arity contradicts its independently observed identity",
                    isConflict = true,
                )
            } else {
                owner
            ...
            DotNetGenericOwnerPhysicalTypeDefEmissionObservation(
                physicalType = checkedOwner,
                physicalKey = localTypeDefPhysicalKeys[raw.physicalType],
                claimedAliases = localTypeDefCandidatesByClassInfo[raw.physicalType]
                    .orEmpty()
                    .map(DotNetGenericOwnerObservedLocalTypeDefCandidate::identity)
                    .fold(mutableListOf<DotNetGenericOwnerPhysicalTypeDefIdentity.Local>()) { distinct, identity ->
                ...
                genericParameters = raw.genericParameters.map { parameter ->
                    DotNetGenericOwnerPhysicalTypeDefGenericParameterObservation(
                        ...
                        parameter.constraints.map { constraint ->
                            normalizeObservedTypeCarrier(constraint, physicalBinder)
                        },
                    )
                },
                directSupertypes = raw.directSupertypes.map { edge ->
                    DotNetGenericOwnerPhysicalTypeDefEdgeObservation(
                        edge.kind,
                        when (val target = edge.target) {
                            ...
                            is DotNetIlRawTypeDefEdgeTarget.Type ->
                                normalizeObservedTypeCarrier(target.carrier, physicalBinder)
                            is DotNetIlRawTypeDefEdgeTarget.Other ->
                                DotNetGenericOwnerObservedMethodCarrier.Other(
        ...
        val methodImplObservations = rawMethodImplObservations.map { raw ->
            val implementingOwner = observedOwner(raw.implementingType)
            val physicalBinder =
                (implementingOwner as? DotNetGenericOwnerObservedMethodDefOwner.Local)?.typeDef
            DotNetGenericOwnerPhysicalMethodImplObservation(
                implementingType = implementingOwner,
                bodyFunction = raw.bodyFunction,
                bodyIdentity = raw.bodyIdentity,
                declarationOwner = normalizeObservedTypeCarrier(
                    raw.declarationOwner,
                    physicalBinder,
                ),
                declarationFunction = raw.declarationFunction,
                declarationIdentity = raw.declarationIdentity,
            )
        }
        return DotNetIlEmissionResult(
            ...
            typeDefEmissionObservations,
            methodImplObservations,
        )
    }
    ...
    private fun checkClassShapeSupported(
        ...
        // Kotlin-owned generic classes have one non-generic CLR owner. KLIB retains their logical
        // parameters while owner-dependent storage and member slots use the erased mapper. A
        // named nested class carries its explicitly normalized logical own/captured parameters
        // until emission; companion-block declarations and a companion singleton use one
        // compiler-owned, non-generic nested holder. A named
        ...
        // Generic base substitution remains logical in IR/KLIB while the emitted base edge names
        // the single erased owner. Override bridges preserve the resulting CLR slot hierarchy.
        ...
        if (irClass.typeParameters.isNotEmpty()) {
            ...
            // Kotlin class variance remains authoritative in KLIB and compiler assignability; it
            // contributes no CLR class variance or class-generic parameter.
    ...
    private fun checkOverrideKeepsIlReturnType(
        ...
        for (overridden in member.allOverridden()) {
            // `allOverridden()` also contains substituted fake views synthesized for inherited
            // declarations. They own no CLR MethodDef; only their resolved declaration can own a
            // physical slot. Comparing the fake view itself would mistake its logical erased
            // Kotlin return for an additional CLR return carrier.
            if (overridden.isFakeOverride) continue
    ...
    private fun renderUserClass(
        ...
        val physicalTypeMapper = typeMapper
        var observedBaseClassType: DotNetIlValueType? = null
        ...
        val baseClassRef = if (irClass.isAnnotationClass) {
            ...
            if (mappedExceptionBase != null) {
                ...
                val baseTypeMapper = physicalTypeMapper
                ...
                when (val baseType = baseTypeMapper.toDotNetIlBaseClassType(baseSuperType)) {
                    is DotNetIlValueType.UserClass -> baseType.ilTypeRef.also {
                        observedBaseClassType = baseType
                    }
                    is DotNetIlValueType.GenericInstance -> baseType.nameInSignature.also {
                        observedBaseClassType = baseType
                    }
                    else -> dotNetUnsupported(
        ...
        val interfaceTypes = (irClass.dotNetDirectInterfaceTypes().mapNotNull { superInterfaceType ->
            ...
            when (interfaceType) {
                ...
                is DotNetIlValueType.GenericInstance,
                    -> interfaceType
                DotNetIlValueType.Object if (genericClassInfo != null) -> null
                else -> dotNetUnsupported(
        ...
        val directTypedInterfaceTypes = if (
            ...
            irClass.dotNetDirectInterfaceTypes().mapNotNull { superInterfaceType ->
                if (
                    ...
                    superInterfaceType.referencesTypeParameterOf(irClass)
                ) {
                    return@mapNotNull null
                }
                val superInterface = (superInterfaceType.classifier as IrClassSymbol).owner
        ...
        val methodImplObservations = if (genericOwnerRehearsal) {
        ...
        for (declaration in irClass.declarations) {
            when (declaration) {
                ...
                is IrClass -> {
                    ...
                    localPlacementObservations?.addAll(rendered.localPlacementObservations)
                    methodDefHeaderObservations?.addAll(rendered.methodDefHeaderObservations)
                    typeDefEmissionObservations?.addAll(rendered.typeDefEmissionObservations)
                    methodImplObservations?.addAll(rendered.methodImplObservations)
                }
                is IrField -> {
        ...
        return RenderedClass(
            ...
            typeDefEmissionObservations.orEmpty(),
    ...
    private fun renderExactGenericInterfaceView(
        ...
        val exactInterfaceTypes = classInfo.interfaces.distinct()
        val emittedGenericParameterDecisions = irClass.typeParameters.dotNetIlGenericParameterDecisions(
            signatureTypeMapper,
    ...
    private class RenderedClass(
        ...
        typeDefEmissionObservations: List<DotNetIlRawTypeDefEmissionObservation>,
        methodImplObservations: List<DotNetIlRawMethodImplObservation>,
    ) {
        ...
        val typeDefEmissionObservations: List<DotNetIlRawTypeDefEmissionObservation> =
            typeDefEmissionObservations.toList()
