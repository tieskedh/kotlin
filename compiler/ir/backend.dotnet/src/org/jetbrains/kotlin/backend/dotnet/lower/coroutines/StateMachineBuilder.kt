...
class SuspendState(type: IrType) {
    val entryBlock: IrContainerExpression = DotNetCoroutineIrBuilder.buildComposite(type)
...
private fun createDispatchPoint(target: SuspendState): IrComposite =
    DotNetCoroutineIrBuilder.buildComposite(target.entryBlock.type).also { it.suspendState = target }
...
/**
 * Target-local adaptation of the explicit ordinary-IR state machine used by JS and by Wasm
 * without stack switching. It deliberately depends only on Common IR plus [DotNetBackendContext]:
 * sharing the algorithm must not make the CLR backend depend on Web compiler infrastructure.
 */
internal class StateMachineBuilder(
    ...
    private val thisReceiver get() = DotNetCoroutineIrBuilder.buildGetValue(thisSymbol)
    ...
    private val globalExceptionVar = DotNetCoroutineIrBuilder.buildVar(exceptionSymbolGetter.returnType.makeNotNull(), function.owner, "e")
    ...
    private fun buildGlobalCatch(): IrCatch {
        ...
        val block = DotNetCoroutineIrBuilder.buildBlock(unit)
        if (hasExceptions) {
            val thenBlock = DotNetCoroutineIrBuilder.buildBlock(unit)
            val elseBlock = DotNetCoroutineIrBuilder.buildBlock(unit)
            val check = DotNetCoroutineIrBuilder.buildCall(eqeqeqSymbol).apply {
                arguments[0] = exceptionState()
                arguments[1] = createDispatchPoint(rootExceptionTrap)
            }
            block.statements += DotNetCoroutineIrBuilder.buildIfElse(unit, check, thenBlock, elseBlock)
            thenBlock.statements += DotNetCoroutineIrBuilder.buildThrow(
                nothing,
                DotNetCoroutineIrBuilder.buildGetValue(globalExceptionSymbol)
            )

            // Exception routing is persisted on the coroutine object because a suspension may
            // leave the CLR invocation that originally entered the protected region.
            elseBlock.statements += DotNetCoroutineIrBuilder.buildCall(stateSymbolSetter.symbol, unit).apply {
                arguments[0] = thisReceiver
                arguments[1] = exceptionState()
            }
            elseBlock.statements += DotNetCoroutineIrBuilder.buildCall(exceptionSymbolSetter.symbol, unit).apply {
                arguments[0] = thisReceiver
                arguments[1] = DotNetCoroutineIrBuilder.buildGetValue(globalExceptionSymbol)
            }
        } else {
            block.statements += DotNetCoroutineIrBuilder.buildThrow(
                nothing,
                DotNetCoroutineIrBuilder.buildGetValue(globalExceptionSymbol)
            )
        }

        return DotNetCoroutineIrBuilder.buildCatch(catchVariable, block)
    }
    ...
    private fun buildExceptionTrapState(): SuspendState {
        ...
        state.entryBlock.statements += DotNetCoroutineIrBuilder.buildThrow(nothing, pendingException())
        return state
    }
    ...
    private fun doDispatchImpl(target: SuspendState, block: IrContainerExpression, andContinue: Boolean) {
        ...
        block.addStatement(DotNetCoroutineIrBuilder.buildCall(stateSymbolSetter.symbol, unit).apply {
            arguments[0] = thisReceiver
    ...
    private fun doContinue(block: IrContainerExpression = currentBlock) {
        block.addStatement(DotNetCoroutineIrBuilder.buildContinue(nothing, rootLoop))
    }
    ...
    private fun buildDispatchBlock(target: SuspendState) = DotNetCoroutineIrBuilder.buildComposite(unit)
    ...
    override fun visitWhileLoop(loop: IrWhileLoop) = transformLoop(loop) { l, head, exit ->
        ...
        transformLastExpression {
            val exitCond = DotNetCoroutineIrBuilder.buildCall(booleanNotSymbol).apply { dispatchReceiver = it }
            val irBreak = buildDispatchBlock(exit)
            DotNetCoroutineIrBuilder.buildIfElse(unit, exitCond, irBreak)
        }
    ...
    override fun visitDoWhileLoop(loop: IrDoWhileLoop) = transformLoop(loop) { l, head, exit ->
        ...
        transformLastExpression {
            ...
            DotNetCoroutineIrBuilder.buildIfElse(unit, it, irContinue)
        }

        doDispatch(exit)
    }

    private fun implicitCast(value: IrExpression, toType: IrType) = DotNetCoroutineIrBuilder.buildImplicitCast(value, toType)
    ...
    override fun visitCall(expression: IrCall) {
        ...
        if (expression.isSuspend) {
            ...
            transformLastExpression {
                DotNetCoroutineIrBuilder.buildCall(stateSymbolSetter.symbol, unit).apply {
                    arguments[0] = thisReceiver
            ...
            val irReturn = DotNetCoroutineIrBuilder.buildReturn(function, getSuspendResultAsType(anyN), nothing)
            val check = DotNetCoroutineIrBuilder.buildCall(eqeqeqSymbol).apply {
                arguments[0] = getSuspendResultAsType(anyN)
                arguments[1] = DotNetCoroutineIrBuilder.buildCall(context.symbols.coroutineSuspendedGetter)
            }

            val suspensionBlock = DotNetCoroutineIrBuilder.buildBlock(unit, listOf(irReturn))
            addStatement(DotNetCoroutineIrBuilder.buildIfElse(unit, check, suspensionBlock))
    ...
    private fun wrap(expression: IrExpression, variable: IrVariableSymbol) =
        DotNetCoroutineIrBuilder.buildSetVariable(variable, expression, unit)
    ...
    override fun visitWhen(expression: IrWhen) {
        ...
        for (branch in branches) {
            if (!isElseBranch(branch)) {
                ...
                val branchBlock = DotNetCoroutineIrBuilder.buildComposite(branch.result.type)
                val elseBlock = DotNetCoroutineIrBuilder.buildComposite(expression.type)

                val dispatchState = currentState
                transformLastExpression {
                    // Keep an explicit else block so later state dispatch has one stable target,
                    // even when subsequent simplification can prove that block empty.
                    DotNetCoroutineIrBuilder.buildIfElse(unit, it, branchBlock, elseBlock)
                }
        ...
        if (varSymbol != null) {
            addStatement(DotNetCoroutineIrBuilder.buildGetValue(varSymbol))
        }
    }
    ...
    override fun visitVariable(declaration: IrVariable) {
        ...
        if (declaration !in suspendableNodes) {
            initializer?.let { addStatement(DotNetCoroutineIrBuilder.buildSetValue(declaration.symbol, it, startOffset, endOffset)) }
            return
        }

        initializer?.acceptVoid(this)
        transformLastExpression { DotNetCoroutineIrBuilder.buildSetValue(declaration.symbol, it, startOffset, endOffset) }
    }
        ...
        arguments.replaceAll { arg ->
            if (arg.isPure(false)) arg else {
                ...
                if (suspendableCount > 0) {
                    ...
                    transformLastExpression {
                        DotNetCoroutineIrBuilder.buildSetValue(irVar.symbol, it)
                    }
                    @Suppress("UNCHECKED_CAST")
                    DotNetCoroutineIrBuilder.buildGetValue(irVar.symbol) as E
                } else {
    ...
    // String concatenation remains explicit here because its arguments may suspend before the
    // later target string-concatenation lowering sees this body.
    ...
    private val unitValue get() = DotNetCoroutineIrBuilder.buildGetObjectValue(unit, context.irBuiltIns.unitClass)
    ...
    override fun visitTry(aTry: IrTry) {
        ...
        // Record the catch destination in state before entering the suspendable try body; a CLR
        // exception table cannot span the separate invocation used for resumption.
        setupExceptionState(tryState.catchState)

        val tryResult = if (varSymbol != null) {
            DotNetCoroutineIrBuilder.buildSetVariable(varSymbol.symbol, aTry.tryResult, unit).also {
        ...
        for (catch in aTry.catches) {
            ...
            val catchResult = if (varSymbol != null) {
                DotNetCoroutineIrBuilder.buildSetVariable(varSymbol.symbol, catch.result, unit).also {
                    if (it.value in suspendableNodes) suspendableNodes += it
                }
            } else catch.result
            ...
            if (type is IrDynamicType) {
                ...
                val branchBlock = DotNetCoroutineIrBuilder.buildComposite(catchResult.type)

                val elseBlock = DotNetCoroutineIrBuilder.buildComposite(catchResult.type)
                val irIf = DotNetCoroutineIrBuilder.buildIfElse(catchResult.type, check, branchBlock, elseBlock)
        ...
        if (rethrowNeeded) {
            addExceptionEdge()
            addStatement(DotNetCoroutineIrBuilder.buildThrow(nothing, pendingException()))
        }
        ...
        if (varSymbol != null) {
            addStatement(DotNetCoroutineIrBuilder.buildGetValue(varSymbol.symbol))
        }
    }

    private fun setupExceptionState(target: SuspendState) {
        addStatement(
            DotNetCoroutineIrBuilder.buildCall(exStateSymbolSetter.symbol, unit).apply {
                arguments[0] = thisReceiver
    ...
    private fun exceptionState() = DotNetCoroutineIrBuilder.buildCall(exStateSymbolGetter.symbol).also { it.dispatchReceiver = thisReceiver }
    private fun pendingException() = DotNetCoroutineIrBuilder.buildCall(exceptionSymbolGetter.symbol).also { it.dispatchReceiver = thisReceiver }
    ...
    private fun buildIsCheck(value: IrExpression, toType: IrType) =
        DotNetCoroutineIrBuilder.buildTypeOperator(
    ...
    private fun tempVar(type: IrType, name: String = "tmp") =
        DotNetCoroutineIrBuilder.buildVar(type, function.owner, name, origin = IrDeclarationOrigin.IR_TEMPORARY_VARIABLE)
            .also(::registerLocal)
