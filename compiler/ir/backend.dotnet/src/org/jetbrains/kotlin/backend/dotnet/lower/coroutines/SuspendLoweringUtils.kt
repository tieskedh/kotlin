...
/** Common-IR dependency analysis adapted from the JS/Wasm explicit state-machine path. */
open class SuspendableNodesCollector(private val suspendableNodes: MutableSet<IrElement>) : IrVisitorVoid() {
...
class LiveLocalsTransformer(
    ...
    override fun visitGetValue(expression: IrGetValue): IrExpression {
        val field = localMap[expression.symbol]
            ?: return when (val owner = expression.symbol.owner) {
                is IrValueParameter -> if (owner.kind == IrParameterKind.DispatchReceiver) receiver() else expression
                else -> expression
            }
    ...
    override fun visitVariable(declaration: IrVariable): IrStatement {
        ...
        return if (initializer != null) {
            ...
            DotNetCoroutineIrBuilder.buildComposite(unitType)
        }
    }
}
