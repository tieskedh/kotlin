...
fun FirNamedFunction.setEqualityBoundTypeFromOverridden(
    ...
    if (isEquals(session)) {
        val bounds = overridden.filterIsInstance<FirNamedFunctionSymbol>().mapNotNull { it.equalityBoundType }
        if (bounds.isNotEmpty()) {
            equalityBoundType = session.typeContext.intersectTypes(bounds)
        }
    }
}
...
fun FirNamedFunction.setEqualityBoundTypeFromOverridden(
    ...
    setEqualityBoundTypeFromOverridden(overridden.map { it.symbol }, session)
}
