...
class FirEqualsOverrideContractCalculator(
    ...
    private fun FirClassSymbol<*>.computeOwnContract(
        ...
        fun processor(function: FirNamedFunctionSymbol): EqualsOverrideContract {
            ...
            val declaringClassId = function.containingClassLookupTag()?.classId ?: return SAFE_FOR_SMART_CAST
            // kotlin.Enum has `equals()`, but we know it's reasonable
            if (declaringClassId == StandardClassIds.Any || declaringClassId == StandardClassIds.Enum) return SAFE_FOR_SMART_CAST
            val declaringClassSymbol = declaringClassId.toSymbol() as? FirClassSymbol ?: return UNKNOWN
    ...
    private fun FirClassSymbol<*>.hasExpectSupertype(symbolsAlreadyCheckedForExpect: MutableSet<FirClassifierSymbol<*>>): Boolean {
        ...
        for (superType in superTypes) {
            if (superType.toClassSymbol()?.resolvedStatus?.isExpect == true) return true
        }
