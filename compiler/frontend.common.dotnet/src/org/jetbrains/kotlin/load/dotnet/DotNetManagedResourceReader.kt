...
/**
 * One CLR DLL classpath entry after a caller-selected managed-carrier discriminator has been read.
 *
 * Presence reports only the requested resource and does not validate its payload or infer its
 * producer. Absence exposes the physical CLR metadata for policy owned by the caller.
 */
sealed interface DotNetClrClasspathAssembly {
    val assemblyFile: File

    data class WithCarrier(
        override val assemblyFile: File,
        val carrierResource: DotNetManagedResource,
    ) : DotNetClrClasspathAssembly

    data class WithoutCarrier(
        override val assemblyFile: File,
...
object DotNetClrClasspathAssemblyReader {
    fun read(file: File, carrierResourceName: String): DotNetClrClasspathAssembly {
...
private object DotNetPeMetadataReader {
    fun readClasspathAssembly(file: File, resourceName: String): DotNetClrClasspathAssembly {
        ...
        return read(file) { image ->
            ...
            if (resource != null) {
                DotNetClrClasspathAssembly.WithCarrier(file, resource)
            } else {
                DotNetClrClasspathAssembly.WithoutCarrier(file, image.readClrMetadata())
            }
        }
    }
