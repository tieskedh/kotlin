...
/**
 * A block tag inside a KDoc comment (such as `@param`, `@return`, or `@throws`) together with the content that
 * follows it.
 *
 * A tag may document a specific entity named by its [subject][getSubjectName], for example the parameter name after
 * `@param`. The primary description of a doc comment, which precedes any block tag, is represented by a nameless tag;
 * see [KDocSection].
 *
 * ### Example:
 *
...
open class KDocTag(node: ASTNode) : KDocElementImpl(node) {
    ...
    /**
     * Returns the [link][KDocLink] naming the entity documented by this tag (for example, the parameter name after
     * `@param`), or `null` if this tag has no subject.
     */
    ...
    /**
     * The [KDocKnownTag] this tag corresponds to, or `null` if the tag name is not recognized or this is the default
     * section.
     */
    ...
    companion object {
        /**
         * The indentation (four spaces) prepended to each line of an indented code block when a tag's
         * [content][getContent] is extracted.
         */
