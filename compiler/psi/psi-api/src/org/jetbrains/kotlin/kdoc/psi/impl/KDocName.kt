...
class KDocName(node: ASTNode) : KtElementImpl(node), KtResolvable {
    ...
    /**
     * Returns the last segment of the name, without the qualifier and without the enclosing backticks if the
     * identifier is escaped (for example, `Bar` for the name `foo.Bar`).
     */
    fun getNameText(): String = getNameTextRange().substring(text)

    /**
     * Returns the segments of the fully qualified name, from the outermost qualifier to this name (for example,
     * `["foo", "Bar"]` for `foo.Bar`).
     */
