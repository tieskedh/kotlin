 ...
 *
 * In the K2 implementation of the Analysis API, the [contentScope] of the library module excludes file types from platforms other than the
 * [targetPlatform]. This can be achieved either with an allowlist or a denylist, depending on the implementation of the restriction scope.
 * For example, a JVM library module filters out `.kotlin_metadata` and `.knm` files, which excludes Kotlin declarations from non-JVM
 * platforms.
 *
